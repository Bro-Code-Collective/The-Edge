      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-0 bottom-0 sm:inset-0 sm:m-auto z-50 w-full sm:max-w-3xl sm:h-fit sm:max-h-[85vh] max-h-[65vh] bg-black text-white rounded-t-[2rem] sm:rounded-[2rem] shadow-elevated overflow-hidden flex flex-col"
          >
            <div className="flex items-center justify-end px-5 pt-5 sm:px-8 sm:pt-8 shrink-0">
              <button
                onClick={onClose}
                aria-label="Close"
                className="w-9 h-9 rounded-full grid place-items-center bg-white/10 hover:bg-white/20 transition-colors focus-dashed"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-6 sm:px-8 sm:pb-8">
              <div className="sm:grid sm:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)] sm:gap-12">
                <div className="hidden sm:block relative sm:w-full aspect-square rounded-2xl overflow-hidden bg-white/10">
                  <Image
                    src={item.image}
                    alt={item.title}
                    fill
                ...
                <div className="space-y-6">
                  <div className="text-center sm:text-left">
                    ...
                    {shopName && (
                      <p className="text-sm text-white/60 mt-0.5">{shopName}</p>
                    )}
                    <div className="mt-1.5 font-mono font-bold text-lg text-white/60">
                  ...
                  <div className="border-t border-white/10 pt-5">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="pill bg-white/10 text-white/60 text-[10px] font-bold uppercase tracking-wider px-3 py-1">
                        Required
                      </span>
                    </div>
                    <p className="text-xs text-white/50 mb-3">Choose 1</p>
                        ...
                        >
                          ...
                          <span
                            className={`w-5 h-5 rounded-full border-2 grid place-items-center transition-colors ${
                              dining === type ? "border-white" : "border-white/30"
                            }`}
                          >
                            {dining === type && <span className="w-2.5 h-2.5 rounded-full bg-white" />}
                          </span>
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="border-t border-white/10 pt-5">
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full text-sm px-4 py-3 rounded-2xl bg-white/10 text-white placeholder:text-white/40 focus:ring-1 focus:ring-white/40 outline-none transition-all resize-none"
                    />
                  </div>
                  ...
                  <div className="border-t border-white/10 pt-5">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="inline-flex items-center rounded-full bg-white/10 overflow-hidden">
                        <button
                          type="button"
                          onClick={() => setQty((q) => Math.max(1, q - 1))}
                          disabled={qty <= 1}
                          className="w-9 h-9 grid place-items-center hover:bg-white/10 transition-colors disabled:opacity-40"
                        >
                    ...
                    {item.maxPerOrder && (
                      <p className="text-[11px] text-white/50 mt-2">Max {item.maxPerOrder} per order</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            >
              <button
                ...
                className="w-full h-14 rounded-2xl bg-white text-black font-bold text-sm hover:bg-white/90 transition-colors"
              >
